...
local redzlib = {
	Themes = {
		...
		RedBlack = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8, 0, 0)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(18, 0, 0)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 0, 0))
			}),
			["Color Hub 2"] = Color3.fromRGB(12, 0, 0),
			["Color Stroke"] = Color3.fromRGB(90, 0, 0),
			["Color Theme"] = Color3.fromRGB(210, 0, 0),
			["Color Text"] = Color3.fromRGB(255, 220, 220),
			["Color Dark Text"] = Color3.fromRGB(180, 100, 100),
			["Color Dark Purple"] = Color3.fromRGB(220, 30, 30),
			["Color Hub 9"] = Color3.fromRGB(0, 0, 0),
			["Color Dark Greem"] = Color3.fromRGB(210, 0, 0)
		}
	},
	Info = {
		Version = "1.2.0"
	...
	Save = {
		UISize = {550, 380},
		TabSize = 160,
		Theme = "RedBlack"   -- ← الثيم الافتراضي الجديد
	},
	Settings = {},
	Connection = {},
	Instances = {},
...
function redzlib:MakeWindow(Configs)
	...
	local function UpdateLogoFGPosition()
		...
		local centerX = (absPos.X + absSize.X / 2) / UIScale
		local centerY = (absPos.Y + absSize.Y / 2) / UIScale
		RotatingLogoFG.Position = UDim2.fromOffset(centerX - 125, centerY - 125)
	end
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddImage(Configs)
			...
			function Image:Destroy() ImageHolder:Destroy() end
			function Image:Set(NewURL)
				if type(NewURL) == "string" then ImageLabel.Image = NewURL end
			end
		...
		function Tab:AddDiscordInvite(Configs)
			...
			local InviteLabel = InsertTheme(Create("TextLabel", InviteHolder, {
				...
				TextColor3 = Color3.fromRGB(210, 0, 0),
				Font = Enum.Font.GothamBold,
				TextXAlignment = "Left",
				BackgroundTransparency = 1,
				TextSize = 9,
			...
			local MainFrame2 = InsertTheme(Create("Frame", InviteHolder, {
			...
			Make("Corner", MainFrame2, UDim.new(0, 12))
			Make("Stroke", MainFrame2)
			
			local BannerArea = Create("ImageLabel", MainFrame2, {
			...
			local ServerIcon = Create("ImageLabel", MainFrame2, {
			...
			local ServerTitle = InsertTheme(Create("TextLabel", MainFrame2, {
			...
			local DescriptionLabel = InsertTheme(Create("TextLabel", MainFrame2, {
				Size = UDim2.new(1, -60, 0, 8),
				Position = UDim2.new(0, 10, MembersOnline > 0 and 0.6 or 0.56, 0),
				TextXAlignment = "Left",
				AutomaticSize = "Y",
				BackgroundTransparency = 1,
				TextSize = 8,
			...
			local BottomSection = InsertTheme(Create("Frame", MainFrame2, {
		...
		function Tab:AddFeedback(Configs)
			...
			local function UpdateStars(n)
				...
				for i, sb in ipairs(StarButtons) do
					sb.ImageColor3 = i <= n and Color3.fromRGB(210, 0, 0) or Theme["Color Stroke"]
				end
			end
			...
			SendBtn.Activated:Connect(function()
				...
				SendBtn.BackgroundColor3 = Color3.fromRGB(120, 0, 0)
